...
#define MEMCHECKREL \
{ \
  int __mem = 0, __memprev = 0, __delta = 0; \
  MEMCHECK_COUNT_MIB(__mem); \
  fraAlgo_ITracker& TRACKER = GetTracker(); \
  __memprev = TRACKER.GetCurrentMem(); \
  __delta   = __mem - __memprev; \
  if ( __delta > 1 ) { std::cout << "(!) Suspicious memory delta " << __delta << "MiB."; } \
  TRACKER.SetCurrentMem(__delta); \
}
